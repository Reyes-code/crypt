...
class KeyHasher:
    def __init__(self):
        self.__private_key, self.__private_keysecond  = PrivateKey().get_private_key()
    
    def hash_combined(self, user_password):
        combined = f"{self.__private_key}{user_password}{self.__private_keysecond}"  # Concatenación
        return generate_password_hash(combined, method='pbkdf2:sha256')

    def verify_combined(self, hashed_combined, user_password):
        combined = f"{self.__private_key}{user_password}{self.__private_keysecond}"
